...
cursor_symbiote는 Cursor IDE의 AI Agent를 전문가 팀처럼 운영하기 위한 자기진화(Self-Evolving) 설정 시스템입니다. `.cursor/` 폴더 하나로 오케스트레이터(Synapse)가 18개의 커스텀 에이전트, 40개의 스킬, 20개의 커맨드, 6개의 훅을 조율하며, 프로젝트가 성장함에 따라 설정 자체도 함께 진화합니다.

이 시스템은 특정 프로젝트 전용이 아닙니다. iOS, Web, Backend 등 어떤 프로젝트에든 `.cursor/` 폴더를 복사하고 `/setup`을 실행하면 코드베이스를 자동 분석하여 프로젝트에 맞게 초기화됩니다.

## 기본 원칙

이 프로젝트는 AI Agent의 생산성보다 사람이 읽기 쉬운 산출물을 우선합니다.

- 이름만 보고 역할이 드러나는 코드
- 하나의 책임만 가진 함수와 파일
- 불필요한 추상화와 설명형 주석 최소화
- 처음 읽는 사람도 안전하게 수정할 수 있는 구조

자세한 기준은 [09-readable-code-principles.md](./09-readable-code-principles.md)를 참조하세요.
...
| `/tm-init` | Task Master 상태 초기화 |
| `/tm-init-manual` | Task Master 수동 초기화 |
| `/tm-parse-prd` | PRD를 task graph로 변환 |
| `/tm-validate` | Task Master 구조 검증 |
| `/tm-board` | task graph 요약 보드 |
| `/tm-next` | 다음 실행 가능한 task 선택 |
| `/tm-start` | 선택한 task 시작 |
| `/tm-sync` | 세션 상태와 task 상태 동기화 |
| `/tm-expand` | task를 subtasks로 분해 |
| `/tm-done` | 완료된 task 반영 |
...
| monitor | 장기 실행 상태 확인, 진행률 추적 | fast | O |
| debugger | 버그 수정, 성능 | inherit | X |
| reviewer | 코드 리뷰 | fast | O |

그 외 architect, build-fixer, designer, doc-writer, migrator, qa-tester, researcher, security-reviewer, tdd-guide, vision 등 역할별 에이전트가 포함됩니다.
