...
오케스트레이터(Synapse)가 18개의 커스텀 에이전트 + 빌트인 subagent, 40개의 스킬, 20개의 커맨드를 조율하며,
...
> 코드 작성 원칙은 [사람이 읽기 쉬운 코드 원칙](Documents/09-readable-code-principles.md)을 참조하세요.
...
2. [디렉터리 구조](#디렉터리-구조)
3. [동작 원리](#동작-원리)
4. [가독성 우선 원칙](#가독성-우선-원칙)
5. [Rules — 룰](#rules--룰)
6. [Commands — 커맨드](#commands--커맨드)
7. [Skills — 스킬](#skills--스킬)
8. [Agents — 서브에이전트](#agents--서브에이전트)
9. [Hooks — 훅](#hooks--훅)
10. [Usage Tracking — 사용 추적](#usage-tracking--사용-추적)
11. [Tests — 테스트](#tests--테스트)
12. [오케스트레이션 시스템](#오케스트레이션-시스템)
13. [자기 진화 메커니즘](#자기-진화-메커니즘)
14. [사용 예시](#사용-예시)
15. [커스터마이징](#커스터마이징)
16. [공식 문서 레퍼런스](#공식-문서-레퍼런스)
17. [상세 문서](#상세-문서)
...
이 프로젝트에서 AI Agent가 작성하는 모든 코드는 먼저 사람이 읽을 수 있어야 합니다.

- 짧은 코드보다 의도가 분명한 코드를 우선합니다.
- 이름만 읽어도 역할이 드러나야 합니다.
- 함수는 하나의 책임만 가져야 합니다.
- 불필요한 주석, 넓은 추상화, 설명 없는 축약은 금지합니다.
- 리뷰 기준에는 동작 여부뿐 아니라 "처음 읽는 사람이 이해 가능한가"가 포함됩니다.

상세 기준은 [Documents/09-readable-code-principles.md](Documents/09-readable-code-principles.md)에 정리되어 있습니다.
...
│   ├── tm-init-manual.md           # Task Master 수동 초기화 절차
│   ├── tm-parse-prd.md             # PRD를 task graph로 정규화
│   ├── tm-validate.md              # Task Master 구조 검증
│   ├── tm-board.md                 # task graph 보드 요약
│   ├── tm-next.md                  # 다음 실행 가능한 task 선택
│   ├── tm-start.md                 # 선택한 task 시작
│   ├── tm-sync.md                  # 세션 상태와 task 상태 동기화
│   ├── tm-expand.md                # task를 subtasks로 분해
│   └── tm-done.md                  # 완료된 task 반영
│
...
| explorer | 읽기 전용 탐색, 실행 경로와 의존성 추적 | fast | O |
...
| monitor | 장기 실행 작업의 상태 추적, 완료/실패 감시 | fast | O |
