...
오케스트레이터(Synapse)가 6개의 커스텀 에이전트 + 빌트인 subagent, 34개의 스킬, 9개의 커맨드를 조율하며,
...
│
├── agents/                         # 커스텀 서브에이전트 (6개, 나머지는 빌트인 위임)
...
커스텀 에이전트 외에 Cursor 빌트인 subagent_type을 활용합니다. `agent-delegation` 룰에 상세 매핑이 정의되어 있습니다.
